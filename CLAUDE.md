...
- 변경: 8개 섹션, 기관 리서치센터 수준 분석으로 업그레이드
...
- 8개 섹션: 핵심 촉발 요인 → 시장 체력 진단(신규) → 자금 흐름 구조 → 금리·달러·유가 연쇄 → 섹터별 강약 → 테마 모멘텀(신규) → 대응 전략(신규) → 다음 트리거+결론
- 프롬프트 강화: 다단계 인과관계 연쇄, 과거 사례 비교, 행동 가능한 시사점, 수치 맥락 부여 요구
- max_tokens: 2048 → 4096, 글자 수 가이드: 1500~2500자
- 프론트엔드: secIcons/secColors에 "체력", "대응", "전략", "트리거", "테마", "모멘텀" 등 신규 키워드 추가

...
- 투자 추천/조언 절대 금지
- "~입니다", "~있습니다" 같은 반복적 문체 금지. 다양한 서술체를 사용할 것
- 같은 내용을 다른 섹션에서 반복하지 말 것
- 숫자는 데이터 그대로 인용

## 아래 예시와 동일한 톤·구조·깊이로 작성할 것:

---예시 시작---
🔥 1) 주요 이슈: 중동발 지정학 리스크 급부상

미국과 이스라엘의 공습 이후 이란 및 헤즈볼라의 반격이 이어지며 갈등이 확대되고 있다. 이 충돌은 단순히 군사적 이슈를 넘어 경제·에너지 시장 전반에 충격파를 주고 있으며, 중동 정세가 전세계 투자심리에 가장 큰 변수로 작용하고 있다.

📉 2) 글로벌 증시: 리스크 오프 강화

미국 선물 지수(S&P500, 나스닥, 다우) 모두 1% 이상 하락 흐름을 보이며 위험자산 기피 심리가 강해졌다. 유럽·아시아 증시도 동반 약세를 보이고 있으며, 공포지수(VIX)가 3개월 최고 수준까지 상승하며 변동성 확대 신호가 나타났다.

🛢️ 3) 유가 급등 → 인플레이션 압력 상승

중동발 불확실성으로 유가가 급등, Brent 유가는 한때 작년 이후 최고 수준 근접까지 치솟았다. 호르무즈 해협 공급 우려가 커지면서 에너지 수급 차질 가능성도 재부각되고 있다.
📌 유가 상승은 기업 생산비·운송비 증가로 이어지며 글로벌 인플레이션 압력을 자극할 가능성이 크다.

📊 4) 섹터별 반응

↘ 부진: 여행·운항·레저 업종은 운항 취소 확산으로 급락. 금융주는 금리·지정학 리스크가 부담.
↗ 강세: 국방·방산 업종은 분쟁 장기화 시 수요 기대감. 에너지 섹터는 유가 급등에 힘입어 상대적 강세.

📈 5) 한국 시장 맥락

코스피 2,600선이 붕괴되며 외국인 매도세가 지속되고 있다. 원/달러 환율 상승(원화 약세)이 수출주에는 긍정적이나 외국인 자금 이탈을 가속화하는 양날의 검으로 작용 중이다.

🧠 6) 핵심 정리

리스크 오프 심리 강세 → 주가 약세
유가 급등 → 인플레 우려 재부각
방산·에너지 강세 vs 여행·금융 약세
📍 내일 주목 포인트: 중동 정세 추가 확전 여부, 미국 고용지표 발표
---예시 끝---

## 절대 하지 말 것 (나쁜 예시):
❌ "상승 섹터로는 2차전지와 철강/소재 섹터가 있습니다. 이들 섹터는 각각 0.88%와 0.02% 상승했습니다."
❌ "거래대금 상위 종목으로는 LG화학, 한미반도체 등이 있습니다."
❌ "오늘 시장의 핵심 포인트는 리스크 오프 심리입니다."
→ 이런 식의 단순 나열, 수치만 읽기, 뻔한 서술은 절대 금지

## 반드시 할 것 (좋은 예시):
✅ "2차전지 섹터는 유럽 전기차 보조금 확대 기대감에 LG에너지솔루션(+3.2%)을 필두로 반등에 성공했다."
✅ "반도체 업종은 미국 엔비디아 실적 우려가 SK하이닉스(-3.46%)를 끌어내리며 약세 흐름을 이어갔다."
✅ "원/달러 환율 1,460원 돌파 → 외국인 자금 이탈 가속 → 코스피 추가 하방 압력"
→ 뉴스와 종목/섹터를 연결하여 원인을 분석하고, 구체적 종목과 수치를 함께 언급

## 위 예시의 톤·구조·깊이를 따를 것:
1. 각 섹션 제목에 이모지 + 번호 + 핵심 키워드
2. "~다", "~있다" 등 간결한 서술체 (존댓말 금지)
3. 원인→결과를 화살표(→)로 연결
4. 📌, 👉 등으로 핵심 포인트 강조
5. 섹터 반응은 ↘/↗ 로 상승/하락 분리하되, 반드시 원인을 함께 서술
6. 마지막 정리에서 핵심 키워드를 한줄씩 정리
7. 1000~1500자 분량

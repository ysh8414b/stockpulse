...
def generate_ai_summary(indices, stocks, sectors, themes, news):
    ...
    prompt = f"""너는 한국 증시 전문 애널리스트다. 아래 시장 데이터를 종합 분석하여 투자자에게 유용한 시장 브리핑을 작성하라.
...
## 작성 규칙:
1. 반드시 한글과 숫자만 사용 — 한자, 일본어, 중국어 절대 사용 금지
2. 이모지를 적극 활용하여 가독성을 높일 것
3. 단순 나열 금지 — 원인과 결과를 연결하여 분석적으로 서술
4. 숫자는 데이터 그대로 정확히 인용
5. 투자 추천/조언 절대 금지

## 반드시 아래 6개 섹션 구성을 따를 것:

1) 주요 이슈 (제목에 핵심 키워드 포함)
- 오늘 시장을 움직이는 가장 큰 이슈 1~2개를 뉴스 기반으로 상세 분석
- 해당 이슈가 시장 전반에 미치는 영향을 원인→결과로 설명
- 3~5문장

2) 글로벌 증시 동향
- 미국(다우·나스닥·S&P500), 환율 흐름과 한국 시장 연관성
- 투자심리(위험자산 선호/회피) 분석
- 2~3문장

3) 주요 종목 분석
- 거래대금 상위 종목 급등/급락 원인과 배경
- 특이 종목 (상한가/하한가, 거래 급증 등) 언급
- 3~4문장

4) 섹터·테마 반응
- 상승 섹터/하락 섹터를 나눠 분석하고 원인 설명
- 인기 테마와 대장주 움직임
- 3~4문장

5) 한국 시장 맥락
- 코스피·코스닥 지수 수치와 등락 배경
- 외국인·기관 수급, 환율 등 국내 고유 요인 분석
- 2~3문장

6) 정리: 오늘 시장의 핵심 포인트
- 확실한 방향성 키워드 3~4개 (예: "리스크 오프 심리 강세 → 주가 약세")
- 내일 주목할 포인트 1~2개
- 3~4문장

## 분량: 1000~1500자 (충분히 자세하게)
